...
namespace AbpSevenDemo.Blazor.Navigation;

public class AbpSevenDemoMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public AbpSevenDemoMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<AbpSevenDemoResource>();

        context.Menu.AddItem(new ApplicationMenuItem(
            AbpSevenDemoMenus.Home,
            l["Menu:Home"],
            "/",
            icon: "fas fa-home",
            order: 1
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                AbpSevenDemoMenus.HostDashboard,
                l["Menu:Dashboard"],
                "/HostDashboard",
                icon: "fa fa-chart-line",
                order: 2
            ).RequirePermissions(AbpSevenDemoPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                AbpSevenDemoMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "/Dashboard",
                icon: "fa fa-chart-line",
                order: 2
            ).RequirePermissions(AbpSevenDemoPermissions.Dashboard.Tenant)
        );
